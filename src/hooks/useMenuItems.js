...
// constants/mocks/menuMock.js の値のみを使う(フロントエンドのみで完結する)。
...
export function useMenuItems(step) {
  // USE_MOCK_MENUがオンの場合は初期値からモック値を使い、
  // 初回描画時に価格未取得の一瞬の表示崩れ(フラッシュ)が起きないようにする。
  const [prices, setPrices] = useState(USE_MOCK_MENU ? MOCK_MENU_PRICES : {});
  const [itemNames, setItemNames] = useState(USE_MOCK_MENU ? MOCK_MENU_ITEM_NAMES : {});
  const [imagePaths, setImagePaths] = useState(USE_MOCK_MENU ? MOCK_MENU_IMAGE_PATHS : {});
